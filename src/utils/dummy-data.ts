// faking a database
// we will never access this file directly, we will use the api
...
export function _savePoll(poll: any) {
  return new Promise((res, rej) => {
...
export function _savePollAnswer(result: {
  authedUser: string;
  id: string;
  answer: string;
}) {
  const { authedUser, id, answer } = result;
